...
ReturnValue Game::internalMoveItemDynamic(Cylinder* fromCylinder, Cylinder* toCylinder, int32_t index, Item* item, uint32_t count, Item** movedItem, uint32_t flags /*= 0*/, Creature* actor /*=nullptr*/, Item* tradeItem /* = nullptr*/, bool checkTile /* = false*/) {
	return internalMoveItem(fromCylinder, toCylinder, index, item, count, movedItem, flags, actor, tradeItem, checkTile);
}
...
ReturnValue Game::processMoveOrAddItemToLootContainer(Item* item, Container* lootContainer, uint32_t &remainderCount, Player* player) {
	...
	ReturnValue ret;
	if (item->getParent()) {
		ret = internalMoveItemDynamic(item->getParent(), lootContainer, INDEX_WHEREEVER, item, item->getItemCount(), &moveItem, 0, player);
	} else {
